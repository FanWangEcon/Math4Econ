...
x_lower_bd = min(-10, max_x-abs(max_x)/2);
x_upper_bd = max(10, max_x+abs(max_x)/2);
% Draw the function
fplot(f, [x_lower_bd, x_upper_bd]);
% Label
xlabel(['X-axis, Quadratic Utility, max U reached at x=', num2str(max_x)])
ylabel(['Utility'])
grid on
%% 
% % 
