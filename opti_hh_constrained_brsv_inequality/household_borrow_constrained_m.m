%% *Borrowing Constrained Utility Maximization with Borrow/Savings Choices*
...
syms beta z1 z2 r
U_neg = @(b) -1*(log(z1 - b) + beta*log(z2 + b*(1+r)))
% Constraint
A = [-1];
q = -b_bar_num;
%% 
...
[b_opti,U_at_b_opti] = fmincon(U_neg_num, b0, A, q);
b_opti
U_at_b_opti
...
b_opti_vec = zeros(size(Z2_vec));
% Solving for optimal choices as we change Z2
for i=1:1:length(Z2_vec)
    U_neg_num = matlabFunction(subs(U_neg, {beta, z1, z2, r}, {beta_num, z1_num, Z2_vec(i), r_num}));
    options = optimoptions('FMINCON','Display','off');
    [b_opti,U_at_b_opti] = fmincon(U_neg_num, b0, A, q, [], [], [], [], [], options);
    b_opti_vec(i) = b_opti;
end

% Plot Results
figure()
plot(Z2_vec, b_opti_vec)
grid on;
ylim([-2 4]);
...
% A vector to store optimal choices
b_opti_vec = zeros(size(r_vec));
% Solving for optimal choices as we change Z2
for i=1:1:length(r_vec)
    U_neg_num = matlabFunction(subs(U_neg, {beta, z1, z2, r}, {beta_num, z1_num, z2_num, r_vec(i)}));
    options = optimoptions('FMINCON','Display','off');
    [b_opti,U_at_b_opti] = fmincon(U_neg_num, b0, A, q, [], [], [], [], [], options);
    b_opti_vec(i) = b_opti;
end
