...
% -1}}$
%% 
% We used these equations to represent supply and demand here: <https://fanwangecon.github.io/Math4Econ/matrix_application/demand_supply_taylor_approximate.html 
...
% I copy below the parameters from <https://fanwangecon.github.io/Math4Econ/derivative_application/K_borrow_firm.html 
% Credit Demand and Supply>
...
exact_r_equi = fzero(matlabFunction(Demand_minus_Supply), 1)
...
b = Z/(2*(1+beta));
%% 
% And now type in the matrix we derived from <https://fanwangecon.github.io/Math4Econ/matrix_application/demand_supply_taylor_approximate.html 
...
hline = refline([0 exact_r_equi]);
hline.Color = 'k';
hline.LineStyle = '-';
hline = refline([0 R_equi_approximate]);
hline.Color = 'k';
hline.LineStyle = '--';
% Legends
...
R_equi_approximate_A = subs(R_equi_approximate, {Z, beta, alpha}, {Z_num, beta_num, alpha_num});
Q_equi_approximate_A = subs(Q_equi_approximate, {Z, beta, alpha}, {Z_num, beta_num, alpha_num});
figure();
subplot(2,2,1);
fplot(R_equi_approximate_A, [2.5, 3.5])
xlabel('A from Firm Problem');
ylabel('Approxi. Equi. R');
title('plot approximate r(A)')
grid on
subplot(2,2,2);
fplot(diff(R_equi_approximate_A, A), [2.5, 3.5])
xlabel('A from Firm Problem');
ylabel('Marginal effect');
title('Derivative d(r(A))/dA ')
grid on
subplot(2,2,3);
fplot(Q_equi_approximate_A, [2.5, 3.5])
xlabel('A from Firm Problem');
ylabel('Approxi. Equi. Q');
title('plot approximate Q(A)')
grid on
subplot(2,2,4);
fplot(diff(Q_equi_approximate_A, A), [2.5, 3.5])
xlabel('A from Firm Problem');
ylabel('Marginal effect');
...
% We can simply use fplot to plot the results out, 
R_equi_approximate_Z = subs(R_equi_approximate, {A, beta, alpha}, {A_num, beta_num, alpha_num});
Q_equi_approximate_Z = subs(Q_equi_approximate, {A, beta, alpha}, {A_num, beta_num, alpha_num});
figure();
subplot(2,2,1);
fplot(R_equi_approximate_Z, [1 30])
xlabel('Z from Household Problem');
ylabel('Approxi. Equi. R');
title('plot approximate r(Z)')
grid on
subplot(2,2,2);
fplot(diff(R_equi_approximate_Z, Z), [1 30])
xlabel('Z from Household Problem');
ylabel('Marginal effect');
title('Derivative d(r(Z))/dZ ')
grid on
subplot(2,2,3);
fplot(Q_equi_approximate_Z, [1 30])
xlabel('Z from Household Problem');
ylabel('Approxi. Equi. Q');
title('plot approximate Q(Z)')
grid on
subplot(2,2,4);
fplot(diff(Q_equi_approximate_Z, Z), [1 30])
xlabel('Z from Household Problem');
ylabel('Marginal effect');
...
% We can simply use fplot to plot the results out, 
R_equi_approximate_beta = subs(R_equi_approximate, {A, Z, alpha}, {A_num, Z_num, alpha_num});
Q_equi_approximate_beta = subs(Q_equi_approximate, {A, Z, alpha}, {A_num, Z_num, alpha_num});
figure();
subplot(2,2,1);
fplot(R_equi_approximate_beta, [0.75, 0.99])
xlabel('beta from Household Problem');
ylabel('Approxi. Equi. R');
title('plot approximate r(beta)')
grid on
subplot(2,2,2);
fplot(diff(R_equi_approximate_beta, beta), [0.75, 0.99])
xlabel('beta from Household Problem');
ylabel('Marginal effect');
title('Derivative d(r(beta))/dbeta ')
grid on
subplot(2,2,3);
fplot(Q_equi_approximate_beta, [0.75, 0.99])
xlabel('beta from Household Problem');
ylabel('Approxi. Equi. Q');
title('plot approximate Q(beta)')
grid on
subplot(2,2,4);
fplot(diff(Q_equi_approximate_beta, beta), [0.75, 0.99])
xlabel('beta from Household Problem');
ylabel('Marginal effect');
...
% We can simply use fplot to plot the results out, 
R_equi_approximate_alpha = subs(R_equi_approximate, {A, Z, beta}, {A_num, Z_num, beta_num});
Q_equi_approximate_alpha = subs(Q_equi_approximate, {A, Z, beta}, {A_num, Z_num, beta_num});
figure();
subplot(2,2,1);
fplot(R_equi_approximate_alpha, [0.30, 0.60])
xlabel('alpha from Household Problem');
ylabel('Approxi. Equi. R');
title('plot approximate r(alpha)')
grid on
subplot(2,2,2);
fplot(diff(R_equi_approximate_alpha, alpha), [0.30, 0.60])
xlabel('alpha from Household Problem');
ylabel('Marginal effect');
title('Derivative d(r(beta))/dalpha ')
grid on
subplot(2,2,3);
fplot(Q_equi_approximate_alpha, [0.30, 0.60])
xlabel('alpha from Household Problem');
ylabel('Approxi. Equi. Q');
title('plot approximate Q(alpha)')
grid on
subplot(2,2,4);
fplot(diff(Q_equi_approximate_alpha, alpha), [0.30, 0.60])
xlabel('alpha from Household Problem');
ylabel('Marginal effect');
