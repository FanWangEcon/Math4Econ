...
core:
- code: 'solve(diff(U, D)==0, diff(U, B)==0, D, B)

    '
  package: m
date: '2020-05-31'
date_start: '2018-12-01'
description: 'Bad and good states of the world.
...
title: Household's Utility Maximization Problem and Investments in Risky and Safe
  Assets in Different States of the World
...
Previously, we have solved the [household savings problem without
uncertainty](https://fanwangecon.github.io/Math4Econ/derivative_application/K_save_households.html).
...
-   **Uncertain:** Even if the chance of having the good economy
...
that compared to our [Two Periods Saving/Borrowing without
Shocks](https://fanwangecon.github.io/Math4Econ/derivative_application/K_save_households.html),
...
2.  If $D>0$ and $B<0$, return in DOW so attractive that you borrow from
    BOA to finance your stock purchases.

3.  If $D=0$ and $B<0$, borrow from BOA to increase consumption today,
...
2.  $(MUC_{2h} \cdot R\cdot \beta \cdot p_h )$ = (marginal utility of
    consumption t=2 in **boom**) x (marginal return to **safe asset**) x
    (time discount) x (probability of **good** state)

3.  $(MUC_{2l} \cdot R\cdot \beta \cdot (1-p_h ))$ = (marginal utility
    of consumption t=2 in **bust**) x (marginal return to **safe
    asset**) x (time discount) x (probability of **bad** state)
...
2.  $(MUC_{2h} \cdot R_h \cdot \beta \cdot p_h )$ = (marginal utility of
    consumption t=2 in **boom**) x (marginal return to **risky asset**)
    x (time discount) x (probability of **good** state)
...
Is there an *upper bound* to this borrowing? Yes, the household knows
...
also called the ***natural borrowing constraint***.
